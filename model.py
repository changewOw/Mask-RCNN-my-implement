...
class DetectionTargetLayer(KE.Layer):
    ...
    def call(self, inputs):
        proposals = inputs[0]     # [batch, N, (y1, x1, y2, x2)]
        gt_class_ids = inputs[1]  # [batch, MAX_GT_INSTANCES]
        gt_boxes = inputs[2]      # [batch, MAX_GT_INSTANCES, (y1, x1, y2, x2)]
        gt_masks = inputs[3]      # [batch, height, width, MAX_GT_INSTANCES]
...
def refine_detections_graph(rois, probs, deltas, window, config):
    ...
    nms_keep = tf.map_fn(nms_keep_map, unique_pre_nms_class_ids,
                         dtype=tf.int64)  # [?, 默认100]：类别，对应的框索引
...
class DetectionLayer(KE.Layer):
    ...
    def call(self, inputs):
        ...
        # Reshape output（显式指明输出tensor的shape，似乎没什么影响）
...
def build_rpn_model(anchor_stride, anchors_per_location, depth):
    ...
    rpn_bbox: [batch, H * W * anchors_per_location, (dy, dx, log(dh), log(dw))] Deltas to be
                applied to anchors.
    """
    input_feature_map = KL.Input(shape=[None, None, depth],   # 输入:1 3 256
...
def rpn_class_loss_graph(rpn_match, rpn_class_logits):
    ...
    # where:input([[true,false],[true,false]])-->[[0,0],[1,0]] two true values & two ranks
    indices = tf.where(K.not_equal(rpn_match, 0))
    # Pick rows that contribute to the loss and filter out the rest.
    rpn_class_logits = tf.gather_nd(rpn_class_logits, indices)
    anchor_class = tf.gather_nd(anchor_class, indices)  # gather_nd 按索引取值
    # Cross entropy loss
...
def rpn_bbox_loss_graph(config, target_bbox, rpn_match, rpn_bbox):
    ...
    # 这段代码理解如下：
    # rpn_match:一张图保存了anchor个置信度，从中选出为1的anchor，
    # rpn_bbox:通过rpn_match索引rpn_bbox
    # target_bbox:在detectionLayer经过处理有zerospadding处理所以要过滤掉这些0padding这就是batchpack的作用
...
class MaskRCNN():
    ...
    def __init__(self, mode, config, model_dir="./checkpoints/"):
        """
        ...
        self.checkpoints_path = "./checkpoints/mask_rcnn_{epoch:03d}.h5"
        # 在这里调用build网络方法
    ...
    def build(self, mode, config):
        ...
        # C5是最顶层输出，经过一次1 1卷积 256filters，得到FPN的P5
        P5 = KL.Conv2D(config.TOP_DOWN_PYRAMID_SIZE, (1, 1), name='fpn_c5p5')(C5)
        P4 = KL.Add(name="fpn_p4add")([
            KL.UpSampling2D(size=(2, 2), name="fpn_p5upsampled")(P5),
            # 又是(1,1)卷积，1.降维。2.缓冲作用防止梯度影响到bottom-up主干网络。3.组合特征
        ...
        # 针对上采样带来的混叠效应，增加卷积层以削弱negative effect
    ...
    def get_anchors(self, image_shape):
        ...
        if not tuple(image_shape) in self._anchor_cache:
            ...
            self.anchors = a
            # Normalize coordinates 归一化注意归一化的实现
    ...
    def train(self, learning_rate, epochs, layers,augmentation=None):
        ...
        train_generator = data_generator(self.config,shuffle=True, augmentation=augmentation,
                                         batch_size=self.config.BATCH_SIZE)
        callbacks = [
            keras.callbacks.TensorBoard(log_dir=self.model_dir,
                                        histogram_freq=0, write_graph=True, write_images=False),
            keras.callbacks.ModelCheckpoint(self.checkpoints_path,
                                            verbose=0, save_weights_only=True),
        ]

        # Train
        log("\nStarting at epoch {}. LR={}\n".format(self.epoch, learning_rate))
        log("Checkpoint Path: {}".format(self.checkpoints_path))
...
def batch_pack_graph(x, counts, num_rows):
    ...
    counts: [batch]
    """
    # 每一层抽取一定数量的boxes，然后连接在一起 return:(None, 4)
    outputs = []
