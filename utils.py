...
def resize_image(image, min_dim=None, max_dim=None, min_scale=None, mode="square"):
    ...
    h, w = image.shape[:2]
    window = (0, 0, h, w) # 图像0index开始 hw并不在图像内部
    scale = 1 # scale factor
    padding = [(0, 0), (0, 0), (0, 0)]
    crop = None
    # 原说法：保证最小边等于IMAGE_MIN_DIM 但是在这基础上要保证最大边小于IMAGE_MAX_DIM

    if min_dim:
        # Scale up but not down
        scale = max(1, min_dim / min(h ,w ))
    if min_scale and scale < min_scale: # 与设定的最小scale比较
        scale = min_scale
    ...
    if mode == "square": # 若为square，则进行padding
...
def generate_anchors(scales, ratios, shape, feature_stride, anchor_stride):
    ...
    # Enumerate shifts in feature space 偏移量 feature_stride为feature_map的步伐
